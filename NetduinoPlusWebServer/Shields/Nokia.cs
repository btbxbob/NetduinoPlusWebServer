...
namespace Nokia.LCDScreens
{
    ...
    public class Nokia_5110
    {
        ...
        // debounce counters
        private byte[] button_count=new byte[5];
        // button status - pressed/released
        private byte[] button_status=new byte[5];
        // button on flags for user program 
        private byte[] button_flag=new byte[5];
        ...
        public Nokia_5110(bool useBacklight, Cpu.Pin latch = Pins.GPIO_PIN_D3, Cpu.Pin backlight = Pins.GPIO_PIN_D9,
            Cpu.Pin reset = Pins.GPIO_PIN_D7, Cpu.Pin dataCommand = Pins.GPIO_PIN_D4)
        {
            //init key counts and status
            for (byte i = 0; i < 5; i++)
            {
                button_count[i] = 0;
                button_status[i] = 0;
